workflow Add-HypervVmToVbrBackup
{
    param (
		$PSCredName,
		$VBRMgmtServer,
	...
	$Result = InlineScript
	{
		...
		$VbrManagedServer = Get-VBRServer | where {$_.Name -eq $HvServer}

        # Find the VM that needs to be added to the backup job on the VBR managed virtualization host
		$VbrVmToBackup = Find-VBRHvEntity -Server $VbrManagedServer -HostsAndVMs -Name $VM

        # Add the VM to the backup job
		Add-VBRHvJobObject -Job $BackupJob -Entities $VbrVmToBackup
